...
const serverCode = /* js */ `
import { createRequestHandler } from "./tmp-adapter";
  ...
  return {
    ...
    async writeBundle() {
      ...
      if (currentCommand === "build" && isSsr) {
        ...
        const relativeServerPath = toPosixPath(
          relative(functionsDirectory, serverPath),
        );

        await writeFile(
          join(functionsDirectory, "remix-server.mjs"),
          generateNetlifyFunction(relativeServerPath),
        );
      }
    },
  };
